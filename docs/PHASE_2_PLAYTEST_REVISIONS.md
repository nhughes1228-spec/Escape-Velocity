...
Verification for this correction: 56 unit tests, typecheck, root production build, unchanged balance report, 5 local browser tests and 2 Pages-subpath browser tests pass. The merge is `6ff4756`; GitHub Actions Pages run `34037890122` passed its build and deploy jobs. A real local browser session confirmed ignition-to-flight presentation, a completed starter flight, Engine/Fuel visual progression, camera translation beyond the initial band, the offscreen launchpad indicator, purchase-preserved completed-flight framing and the visible Settings/New game controls. A real browser opened the live URL after deployment, observed the header and ignition sequence, settled a 162 m / 19-Credit launch, and reported no application console errors. Final Phase 2 acceptance remains pending Astra review.
