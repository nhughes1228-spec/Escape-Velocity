...
Latest review, 2026-09-06: the deployed presentation correction `07c205d` received an [Astra conditional pass](PHASE_2_PRESENTATION_REVIEW.md). Camera/history behavior and independent checks pass. New game keyboard focus/Escape behavior needs a narrow correction; changed-experience acceptance and the separately versioned Ignition candidate remain pending. Historical checkpoint/deployment details follow.

Status: controller/UI integration deployed to `main`, 2026-09-05. Correction commit `059aee9` passed the focused Astra P0 gate. Merge commit `5a69810` publishes the approved controller, Credits loop, four upgrade cards, historical replay and Settings recovery behavior through GitHub Pages. Automated verification and a live browser smoke pass; Two human sessions have since been reported. Final acceptance is held for the [playtest presentation/usability revisions](PHASE_2_PLAYTEST_REVISIONS.md); the runtime Ignition values are unchanged. [PHASE_2_SPEC.md](PHASE_2_SPEC.md) remains the authoritative design specification.
...
No staging, orbit, payloads, missions, prestige, research tree, secondary currency, failures, engine families, auto-launch, offline income, fast-forward or variance-reduction technology is included. Phase 3 has not begun. The live build remains a bounded Phase 2 opening sandbox. Two reported sessions led to the [presentation/usability improvement handoff](PHASE_2_PLAYTEST_REVISIONS.md); those fixes and final acceptance remain pending.
