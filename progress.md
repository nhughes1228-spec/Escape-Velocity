...
- Astra’s minor stars, offscreen-label and historical-heading follow-ups remain intentionally deferred. PR #9 merged this correction to `main` as `2122dde`; Pages run `34042526093` passed, and the deployed dialog was opened in a real browser. It focused Cancel, trapped Tab/Shift+Tab, closed with Escape, restored focus to New game, preserved 57 Credits and produced no browser console errors. A short revised-presentation human check remains the next product step; the separate versioned Ignition candidate is not being started.
